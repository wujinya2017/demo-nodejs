 ...
 * - logFile {string} master进程的pid保存文件。
 * - errorLogFile {string}
...
var titbit = function (options = {}) {
    ...
    this.limit = {
        ...
        maxIPCache      : 50000, //存储IP最大个数，是req_ip_table的上限，否则于性能有损。
        allow           : [], //限制IP请求次数的白名单。
    };
...
titbit.prototype.showLoadInfo = function (w) {
    ...
    if (this.loadInfo.length >= total) {
        ...
        if (this.config.daemon) {
            try {
                fs.writeFileSync(this.config.loadInfoFile,oscpu+cols, 
                    {encoding:'utf8'});
            } catch (err) { }
        } else {
...
titbit.prototype.daemon = function(port = 2020, host = '0.0.0.0', num = 0) {
    ...
    if (cluster.isMaster) {
        ...
        if (this.config.daemon && this.config.pidFile.length == 0) {
            this.config.pidFile = './titbit.pid';
        }
