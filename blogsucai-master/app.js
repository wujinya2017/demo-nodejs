...
http.createServer((req,res)=>{
  var path = url.parse(req.url).href.split('/');
  var item = path.slice(2);
  var pathh = "./"+item.join('/');
  var pathn = item[item.length-1];
  ...
  if(req.url =='/list/'){
  ...
  else if(req.url === '/list/detail'){
    var data = fs.readFileSync("./chapter.html","utf8");
    res.writeHead(200,{"Content-type":"text/html;charset=UTF-8"}); 
    if(req.method == "GET" && req.url == '/getDetail?chapterId=1'){
      var str = url.parse(req.url).path.split('=')[1];
      var body = JSON.stringify(chapterList[str-1]);
      res.end(body);
      return;
    }

    if(req.method == "GET" && req.url == '/getDetail?chapterId=2'){
      var str = url.parse(req.url).path.split('=')[1];
      var body = JSON.stringify(chapterList[str-1]);
      res.end(body);
      return;
    ...
    if(req.method == "GET" && req.url == '/getDetail?chapterId=3'){
      var str = url.parse(req.url).path.split('=')[1];
      var body = JSON.stringify(chapterList[str-1]);
      res.end(body);
      return;
    ...
    if(req.method == "GET" && req.url == '/getDetail?chapterId=4'){
      var str = url.parse(req.url).path.split('=')[1];
      var body = JSON.stringify(chapterList[str-1]);
      res.end(body);
      return;
  ...
  if(pathn){
      var itempath = pathn.split('.')[1];                     
      switch(itempath){
        case 'css':
            var data=fs.readFileSync(pathh);
